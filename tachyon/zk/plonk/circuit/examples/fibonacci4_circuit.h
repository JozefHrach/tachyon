#ifndef TACHYON_ZK_PLONK_CIRCUIT_EXAMPLES_FIBONACCI_CIRCUIT4_H_
...
class FibonacciChip4 {
 public:
  ...
  static FibonacciConfig4<F> Configure(ConstraintSystem<F>& meta) {
    ...
    FibonacciConfig4<F> config(std::move(advice), s_add, s_xor,
                  ...
                  "advice", config_.advice[2], row, [&b_cell, &c_cell]() {
                    return b_cell.value().template AndThen<F>(
                        [&b = c_cell.value().value()](const F& a) {
                          uint64_t a_val = a[0] & 0xffffffffU;
                          uint64_t b_val = b[0] & 0xffffffffU;
